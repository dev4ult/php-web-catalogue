<form action="" method="post"
    ...
    <div class="m-auto bg-slate-800 p-5 rounded-lg">
        <div class="">
            <h3 for="category-name" class="text-xl font-semibold text-red-500">Warning :</h3>
            <h3 for="category-name" class="text-xl font-semibold text-red-500">Deleting a category may include deleting
                a
                product</h3>
        ...
        <div class="mt-4 relative p-4 z-10">
            <input type="submit" name="confirm-delete" value="Yes"
                class="confirm-delete text-xl rounded-lg text-white px-2 py-0.5 bg-green-400 font-semibold ml-2 shadow-btn shadow-green-500  active:shadow-none transition-all absolute right-0 -translate-x-[100%] -top-0.5 active:top-1 cursor-pointer">
            <a
...
<?php
    if(isset($_POST['confirm-delete'])) {
        ...
        $deleteProduct = mysqli_query($conn, "DELETE FROM product WHERE category_id = '".$_GET['idc']."'");
        $delete = mysqli_query($conn, "DELETE FROM category WHERE category_id = '". $_GET['idc'] ."'");
        
        if($delete) {
            echo '<script>window.location="category.php"</script>';
        } else {
            echo '<script>alert("Error when trying to delete Category"</script>';
        }
    }
?>